...
object SomeApp extends IOApp {
  def run(args: List[String]): IO[ExitCode] =
    for {
      a <- Something[IO].foo(1)
      _ <- putStrLn(a.show)

      b <- Something[IO].bar(2.some)
      _ <- putStrLn(b.show)
      
      c <- Something[IO].bar(none[String])
      _ <- putStrLn(c.show)

      d <- Something[IO].baz(2.asRight[String])
      _ <- putStrLn(d.show)
      
      e <- Something[IO].baz("ERROR!!!".asLeft[Int])
      _ <- putStrLn(e.show)
    } yield ExitCode.Success
}


SomeApp.main(Array.empty)
```
