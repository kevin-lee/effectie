...
trait CanCatch[F[_]] extends effectie.CanCatch[F] {
  ...
  override def catchNonFatalEitherT[A, AA >: A, B](
    fab: => EitherT[F, A, B]
  )(f: Throwable => AA): EitherT[F, AA, B] =
...
object CanCatch {
  ...
  implicit val canCatchIo: CanCatch[Task] = new CanCatch[Task] {
    ...
    override def catchNonFatalEither[A, AA >: A, B](
      fab: => Task[Either[A, B]]
    )(f: Throwable => AA): Task[Either[AA, B]] =
  ...
  final class CanCatchFuture(val EC0: ExecutionContext) extends CanCatch[Future] {
    ...
    )(f: Throwable => AA): Future[Either[AA, B]] =
