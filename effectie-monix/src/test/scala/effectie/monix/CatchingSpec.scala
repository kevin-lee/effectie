...
object CatchingSpec extends Properties {
  ...
  override def tests: List[Test] = List(
    /* Task */
    example("test Catching.catchNonFatal[Task] should catch NonFatal", TaskSpec.testCatching_Task_catchNonFatalShouldCatchNonFatal),
    example("test Catching.catchNonFatal[Task] should not catch Fatal",TaskSpec.testCatching_Task_catchNonFatalShouldNotCatchFatal),
    example("test Catching.catchNonFatal[Task] should return the successful result", TaskSpec.testCatching_Task_catchNonFatalShouldReturnSuccessfulResult),

    example("test Catching.catchNonFatalF[Task] should catch NonFatal", TaskSpec.testCatching_Task_catchNonFatalFShouldCatchNonFatal),
    example("test Catching.catchNonFatalF[Task] should not catch Fatal", TaskSpec.testCatching_Task_catchNonFatalFShouldNotCatchFatal),
    example("test Catching.catchNonFatalF[Task] should return the successful result", TaskSpec.testCatching_Task_catchNonFatalFShouldReturnSuccessfulResult),

    example("test Catching.catchNonFatalEither[Task] should catch NonFatal", TaskSpec.testCatching_Task_catchNonFatalEitherShouldCatchNonFatal),
    example("test Catching.catchNonFatalEither[Task] should not catch Fatal", TaskSpec.testCatching_Task_catchNonFatalEitherShouldNotCatchFatal),
    example("test Catching.catchNonFatalEither[Task] should return the successful result", TaskSpec.testCatching_Task_catchNonFatalEitherShouldReturnSuccessfulResult),
    example("test Catching.catchNonFatalEither[Task] should return the failed result", TaskSpec.testCatching_Task_catchNonFatalEitherShouldReturnFailedResult),

    example("test Catching.catchNonFatalEitherF[Task] should catch NonFatal", TaskSpec.testCatching_Task_catchNonFatalEitherFShouldCatchNonFatal),
    example("test Catching.catchNonFatalEitherF[Task] should not catch Fatal", TaskSpec.testCatching_Task_catchNonFatalEitherFShouldNotCatchFatal),
    example("test Catching.catchNonFatalEitherF[Task] should return the successful result", TaskSpec.testCatching_Task_catchNonFatalEitherFShouldReturnSuccessfulResult),
    example("test Catching.catchNonFatalEitherF[Task] should return the failed result", TaskSpec.testCatching_Task_catchNonFatalEitherFShouldReturnFailedResult),

    example("test Catching.catchNonFatalEitherT[Task] should catch NonFatal", TaskSpec.testCatching_Task_catchNonFatalEitherTShouldCatchNonFatal),
    example("test Catching.catchNonFatalEitherT[Task] should not catch Fatal", TaskSpec.testCatching_Task_catchNonFatalEitherTShouldNotCatchFatal),
    example("test Catching.catchNonFatalEitherT[Task] should return the successful result", TaskSpec.testCatching_Task_catchNonFatalEitherTShouldReturnSuccessfulResult),
    example("test Catching.catchNonFatalEitherT[Task] should return the failed result", TaskSpec.testCatching_Task_catchNonFatalEitherTShouldReturnFailedResult),
  ...
  object TaskSpec {
    ...
    def testCatching_Task_catchNonFatalEitherFShouldCatchNonFatal: Result = {
      ...
      val expectedExpcetion = new RuntimeException("Something's wrong")
      val expected = SomeError.someThrowable(expectedExpcetion).asLeft[Int]
      val actual = catchNonFatalEitherF[Task](throwThrowable[Either[SomeError, Int]](expectedExpcetion))(SomeError.someThrowable).runSyncUnsafe()
    ...
    def testCatching_Task_catchNonFatalEitherFShouldNotCatchFatal: Result = {
      ...
      try {
        val actual = catchNonFatalEitherF[Task](throwThrowable[Either[SomeError, Int]](fatalExpcetion))(SomeError.someThrowable).runSyncUnsafe()
        Result.failure.log(s"The expected fatal exception was not thrown. actual: ${actual.toString}")
      } catch {
        case ex: ControlThrowable =>
          ex ==== fatalExpcetion
  ...
  object FutureSpec {
    ...
    def testCatching_Future_catchNonFatalShouldCatchNonFatal: Result = {
      ...
      val expected = SomeError.someThrowable(expectedExpcetion).asLeft[Int]
      val actual = ConcurrentSupport.futureToValue(catchNonFatal(fa)(SomeError.someThrowable), waitFor)
    ...
    def testCatching_Future_catchNonFatalFShouldCatchNonFatal: Result = {
      ...
      val expectedExpcetion = new RuntimeException("Something's wrong")
      val expected = SomeError.someThrowable(expectedExpcetion).asLeft[Int]
      val actual = ConcurrentSupport.futureToValue(catchNonFatalF[Future](throwThrowable[Int](expectedExpcetion))(SomeError.someThrowable), waitFor)
    ...
    def testCatching_Future_catchNonFatalFShouldReturnSuccessfulResult: Result = {
      ...
      val expected = 1.asRight[SomeError]
      val actual = ConcurrentSupport.futureToValue(catchNonFatalF[Future](1)(SomeError.someThrowable), waitFor)
    ...
    def testCatching_Future_catchNonFatalEitherShouldCatchNonFatal: Result = {
      ...
      val expected = SomeError.someThrowable(expectedExpcetion).asLeft[Int]
      val actual = ConcurrentSupport.futureToValue(catchNonFatalEither(fa)(SomeError.someThrowable), waitFor)
