...
object CanHandleErrorSpec extends Properties {
  ...
  override def tests: List[Test] = List(
    /* Task */
    example("test CanHandleError[Task].handleNonFatalWith should handle NonFatal", TaskSpec.testCanHandleError_Task_handleNonFatalWithShouldHandleNonFatalWith),
    example("test CanHandleError[Task].handleNonFatalWith should not handle Fatal", TaskSpec.testCanHandleError_Task_handleNonFatalWithShouldNotHandleFatalWith),
    example("test CanHandleError[Task].handleNonFatalWith should return the successful result", TaskSpec.testCanHandleError_Task_handleNonFatalWithShouldReturnSuccessfulResult),

    example("test CanHandleError[Task].handleNonFatalWithEither should handle NonFatal", TaskSpec.testCanHandleError_Task_handleNonFatalWithEitherShouldHandleNonFatalWith),
    example("test CanHandleError[Task].handleNonFatalWithEither should not handle Fatal", TaskSpec.testCanHandleError_Task_handleNonFatalWithEitherShouldNotHandleFatalWith),
    example("test CanHandleError[Task].handleNonFatalWithEither should return the successful result", TaskSpec.testCanHandleError_Task_handleNonFatalWithEitherShouldReturnSuccessfulResult),
    example("test CanHandleError[Task].handleNonFatalWithEither should return the failed result", TaskSpec.testCanHandleError_Task_handleNonFatalWithEitherShouldReturnFailedResult),

    example("test CanHandleError[Task].handleEitherTNonFatalWith should handle NonFatal", TaskSpec.testCanHandleError_Task_handleEitherTNonFatalWithShouldHandleNonFatalWith),
    example("test CanHandleError[Task].handleEitherTNonFatalWith should not handle Fatal", TaskSpec.testCanHandleError_Task_handleEitherTNonFatalWithShouldNotHandleFatalWith),
    example("test CanHandleError[Task].handleEitherTNonFatalWith should return the successful result", TaskSpec.testCanHandleError_Task_handleEitherTNonFatalWithShouldReturnSuccessfulResult),
    example("test CanHandleError[Task].handleEitherTNonFatalWith should return the failed result", TaskSpec.testCanHandleError_Task_handleEitherTNonFatalWithShouldReturnFailedResult),

    example("test CanHandleError[Task].handleNonFatal should handle NonFatal", TaskSpec.testCanHandleError_Task_handleNonFatalShouldHandleNonFatal),
    example("test CanHandleError[Task].handleNonFatal should not handle Fatal", TaskSpec.testCanHandleError_Task_handleNonFatalShouldNotHandleFatal),
    example("test CanHandleError[Task].handleNonFatal should return the successful result", TaskSpec.testCanHandleError_Task_handleNonFatalShouldReturnSuccessfulResult),

    example("test CanHandleError[Task].handleNonFatalEither should handle NonFatal", TaskSpec.testCanHandleError_Task_handleNonFatalEitherShouldHandleNonFatal),
    example("test CanHandleError[Task].handleNonFatalEither should not handle Fatal", TaskSpec.testCanHandleError_Task_handleNonFatalEitherShouldNotHandleFatal),
    example("test CanHandleError[Task].handleNonFatalEither should return the successful result", TaskSpec.testCanHandleError_Task_handleNonFatalEitherShouldReturnSuccessfulResult),
    example("test CanHandleError[Task].handleNonFatalEither should return the failed result", TaskSpec.testCanHandleError_Task_handleNonFatalEitherShouldReturnFailedResult),

    example("test CanHandleError[Task].handleEitherTNonFatal should handle NonFatal", TaskSpec.testCanHandleError_Task_handleEitherTNonFatalShouldHandleNonFatal),
    example("test CanHandleError[Task].handleEitherTNonFatal should not handle Fatal", TaskSpec.testCanHandleError_Task_handleEitherTNonFatalShouldNotHandleFatal),
    example("test CanHandleError[Task].handleEitherTNonFatal should return the successful result", TaskSpec.testCanHandleError_Task_handleEitherTNonFatalShouldReturnSuccessfulResult),
    example("test CanHandleError[Task].handleEitherTNonFatal should return the failed result", TaskSpec.testCanHandleError_Task_handleEitherTNonFatalShouldReturnFailedResult),
  ...
  object TaskSpec {
    ...
    def testCanHandleError_Task_handleNonFatalWithShouldHandleNonFatalWith: Result = {
      ...
      val expected = 123
      val actual = CanHandleError[Task].handleNonFatalWith(fa) {
    ...
    def testCanHandleError_Task_handleNonFatalWithEitherShouldHandleNonFatalWith: Result = {
      ...
      val actualFailedResult = CanHandleError[Task].handleNonFatalWith(fa)(_ => Task.pure(expectedFailedResult)).runSyncUnsafe()

      val expectedSuccessResult = 1.asRight[SomeError]
      val actualSuccessResult = CanHandleError[Task].handleNonFatalWith(fa)(_ => Task.pure(1.asRight[SomeError])).runSyncUnsafe()
    ...
    def testCanHandleError_Task_handleEitherTNonFatalWithShouldHandleNonFatalWith: Result = {
      ...
      val actualFailedResult = CanHandleError[Task].handleEitherTNonFatalWith(fa)(err => Task.pure(SomeError.someThrowable(err).asLeft[Int])).value.runSyncUnsafe()
      val expectedSuccessResult = 123.asRight[SomeError]
      val actualSuccessResult = CanHandleError[Task].handleEitherTNonFatalWith(fa)(_ => Task.pure(123.asRight[SomeError])).value.runSyncUnsafe()
    ...
    def testCanHandleError_Task_handleEitherTNonFatalWithShouldNotHandleFatalWith: Result = {
      ...
      try {
        val actual =
          CanHandleError[Task].handleEitherTNonFatalWith(fa)(
            err => Task.pure(SomeError.someThrowable(err).asLeft[Int])
          ).value.runSyncUnsafe()
        Result.failure.log(s"The expected fatal exception was not thrown. actual: ${actual.toString}")
      } catch {
        case ex: ControlThrowable =>
    ...
    def testCanHandleError_Task_handleEitherTNonFatalWithShouldReturnSuccessfulResult: Result = {
      ...
      val expected = 1.asRight[SomeError]
      val actual = CanHandleError[Task].handleEitherTNonFatalWith(fa)(_ => Task.pure(123.asRight[SomeError])).value.runSyncUnsafe()
    ...
    def testCanHandleError_Task_handleNonFatalShouldHandleNonFatal: Result = {
      ...
      val fa = run[Task, Int](throwThrowable[Int](expectedExpcetion))
      val expected = 123
      val actual = CanHandleError[Task].handleNonFatal(fa) {
        case NonFatal(`expectedExpcetion`) =>
          expected
      }.runSyncUnsafe()
    ...
    def testCanHandleError_Task_handleEitherTNonFatalShouldHandleNonFatal: Result = {
      ...
      val fa = EitherT(run[Task, Either[SomeError, Int]](throwThrowable[Either[SomeError, Int]](expectedExpcetion)))
      val expectedFailedResult = SomeError.someThrowable(expectedExpcetion).asLeft[Int]
      val actualFailedResult = CanHandleError[Task].handleEitherTNonFatal(fa)(err => SomeError.someThrowable(err).asLeft[Int]).value.runSyncUnsafe()
      val expectedSuccessResult = 123.asRight[SomeError]
      val actualSuccessResult = CanHandleError[Task].handleEitherTNonFatal(fa)(_ => 123.asRight[SomeError]).value.runSyncUnsafe()
    ...
    def testCanHandleError_Task_handleEitherTNonFatalShouldNotHandleFatal: Result = {
      ...
      try {
        val actual =
          CanHandleError[Task].handleEitherTNonFatal(fa)(
  ...
  object FutureSpec {
    ...
    def testCanHandleError_Future_handleNonFatalWithShouldHandleNonFatalWith: Result = {
      ...
      val expected = 1
      val actual = futureToValueAndTerminate[Int](CanHandleError[Future].handleNonFatalWith(fa)(_ => Future(expected)))
    ...
    def testCanHandleError_Future_handleNonFatalWithShouldReturnSuccessfulResult: Result = {
      ...
      val expected = 1
      val actual = futureToValueAndTerminate(CanHandleError[Future].handleNonFatalWith(fa)(_ => Future(123)))
    ...
    def testCanHandleError_Future_handleNonFatalWithEitherShouldHandleNonFatalWith: Result = {
      ...
      val actualFailedResult =
        futureToValue(CanHandleError[Future].handleNonFatalWith(fa)(err => Future(SomeError.someThrowable(err).asLeft[Int])))

      val fa2 = run[Future, Either[SomeError, Int]](throwThrowable[Either[SomeError, Int]](expectedExpcetion))
      val expected = 1.asRight[SomeError]
      val actual = futureToValueAndTerminate(CanHandleError[Future].handleNonFatalWith(fa2)(_ => Future(expected)))
    ...
    def testCanHandleError_Future_handleNonFatalWithEitherShouldReturnSuccessfulResult: Result = {
      ...
      val expected = 1.asRight[SomeError]
      val actual =
        futureToValueAndTerminate(CanHandleError[Future].handleNonFatalWith(fa)(err => Future(SomeError.someThrowable(err).asLeft[Int])))
    ...
    def testCanHandleError_Future_handleNonFatalWithEitherShouldReturnFailedResult: Result = {
      ...
      val expected = expectedFailure.asLeft[Int]
      val actual = futureToValueAndTerminate(CanHandleError[Future].handleNonFatalWith(fa)(_ => Future(1.asRight[SomeError])))
    ...
    def testCanHandleError_Future_handleEitherTNonFatalWithShouldHandleNonFatalWith: Result = {
      ...
      val expected = 1.asRight[SomeError]
      val actual =
        futureToValueAndTerminate(CanHandleError[Future].handleEitherTNonFatalWith(fa2)(err => Future(expected)).value)
    ...
    def testCanHandleError_Future_handleEitherTNonFatalWithShouldReturnFailedResult: Result = {
      ...
      val expected = expectedFailure.asLeft[Int]
      val actual =
        futureToValueAndTerminate(CanHandleError[Future].handleEitherTNonFatalWith(fa)(_ => Future(expected)).value)
    ...
    def testCanHandleError_Future_handleNonFatalShouldHandleNonFatal: Result = {
      ...
      val fa = run[Future, Int](throwThrowable[Int](expectedExpcetion))
      val expected = 1
      val actual = futureToValueAndTerminate[Int](CanHandleError[Future].handleNonFatal(fa)(_ => expected))
    ...
    def testCanHandleError_Future_handleNonFatalShouldReturnSuccessfulResult: Result = {
      ...
      val fa = run[Future, Int](1)
      val expected = 1
      val actual = futureToValueAndTerminate(CanHandleError[Future].handleNonFatal(fa)(_ => 123))
    ...
    def testCanHandleError_Future_handleNonFatalEitherShouldHandleNonFatal: Result = {
      ...
      val expectedFailedResult = SomeError.someThrowable(expectedExpcetion).asLeft[Int]
      val actualFailedResult =
        futureToValue(CanHandleError[Future].handleNonFatal(fa)(err => SomeError.someThrowable(err).asLeft[Int]))

      val fa2 = run[Future, Either[SomeError, Int]](throwThrowable[Either[SomeError, Int]](expectedExpcetion))
      val expected = 1.asRight[SomeError]
      val actual = futureToValueAndTerminate(CanHandleError[Future].handleNonFatal(fa2)(_ => expected))
    ...
    def testCanHandleError_Future_handleNonFatalEitherShouldReturnSuccessfulResult: Result = {
      ...
      val expected = 1.asRight[SomeError]
      val actual =
        futureToValueAndTerminate(CanHandleError[Future].handleNonFatal(fa)(err => SomeError.someThrowable(err).asLeft[Int]))
    ...
    def testCanHandleError_Future_handleNonFatalEitherShouldReturnFailedResult: Result = {
      ...
      val fa = run[Future, Either[SomeError, Int]](expectedFailure.asLeft[Int])
      val expected = expectedFailure.asLeft[Int]
      val actual = futureToValueAndTerminate(CanHandleError[Future].handleNonFatal(fa)(_ => 1.asRight[SomeError]))
    ...
    def testCanHandleError_Future_handleEitherTNonFatalShouldHandleNonFatal: Result = {
      ...
      val expected = 1.asRight[SomeError]
      val actual =
        futureToValueAndTerminate(CanHandleError[Future].handleEitherTNonFatal(fa2)(err => expected).value)
    ...
    def testCanHandleError_Future_handleEitherTNonFatalShouldReturnFailedResult: Result = {
      ...
      val expected = expectedFailure.asLeft[Int]
      val actual =
        futureToValueAndTerminate(CanHandleError[Future].handleEitherTNonFatal(fa)(_ => expected).value)
