...
object Specs {
  ...
  object functorLaws {
    def laws[F[_]](
      genM: Gen[F[Int]],
      genF: Gen[Int => Int]
    )(implicit functor: Functor[F], eqM: Equal[F[Int]]): Property = for {
      m  <- genM.log("m: F[Int]")
  ...
  object applicativeLaws {
    def laws[F[_]](
      ...
      genInt: Gen[Int],
      genF: Gen[Int => Int]
    )(implicit applicative: Applicative[F], eqM: Equal[F[Int]]): Property = for {
      m  <- genM.log("m: F[Int]")
      x  <- genInt.log("x: Int")
      f  <- genF.log("f: Int => Int")
      f2 <- genF.log("f2: Int => Int")
  ...
  object monadLaws {
    def laws[F[_]](
      ...
      genF: Gen[Int => Int],
      genFm: Gen[Int => F[Int]]
    )(implicit monad: Monad[F], eqM: Equal[F[Int]]): Property = for {
      m   <- genM.log("m: F[Int]")
      x   <- genInt.log("x: Int")
      f   <- genF.log("f: Int => Int")
      f2  <- genF.log("f2: Int => Int")
